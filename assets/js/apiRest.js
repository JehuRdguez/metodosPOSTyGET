function login() {
	...
	$.ajax
		({
			type: "POST",
			url: "/metodosPOSTyGET/auth.php",
			data: $('#login').serialize(),
			success: function (data) {
				if (data==1) {
					window.location.href = "/metodosPOSTyGET/views/users.php";
				}
...
function getUsers() {
	/*let items = "";
		...
		$.get("/metodosPOSTyGET/APIREST/users.php", 
			function (data, status) {
	...
	$.ajax({
		url: "/metodosPOSTyGET/APIREST/users.php",
		type: 'GET',
	...
	/*fetch("/metodosPOSTyGET/APIREST/users.php")
...
function changeStatus(idUser) {
	...
	/*$.get("/metodosPOSTyGET/APIREST/users.php",{name : name} ,
		function(data, status){
			alert(data);
	});*/
}
